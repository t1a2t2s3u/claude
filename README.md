...
`cover` に一覧用のサムネイルを指定します。**全記事に揃っていないと一覧の写真が
1枚も出ない**仕組みなので、記事を足すときは必ず設定してください（揃っていないと
`site-build --strict` が警告で止まります）。

記事の挿絵・サムネイルは、施工事例と違って**フリー素材を使っても構いません**。
実績を示す写真ではないためです。ただし利用規約を確認し、透かし入りの画像は
使わないこと。実際の現場写真があるなら、そちらのほうが他社と差がつきます。
