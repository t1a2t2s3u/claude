...
ランディングページ）は、`https://tatsumi-tosou.com` へ 301 転送する。

**理由は評価の分散ではなく、情報の食い違い。** 会社名・住所・電話番号は
新サイトと同じなのに、営業時間（土曜）と対応エリア（「秋田県内」）が
違っていた。同じ事業所を指す2つのページが違うことを書いている状態は、
MEO の減点材料になる。集客の実績も無く、残す理由が無い。

- **ドメインは手放さない。** 転送のために保持する
- 移したいコンテンツは無い（トップの人物写真はフリー素材だった）
- 広告の着地ページとしては使っていないので、`noindex` で共存させる
  必要も無い

転送は Cloudflare で行う。ネームサーバーを Cloudflare に向けてから
**Rules → Redirect Rules** で `https://tatsumi-tosou.com` へ 301。
**LP のサーバーを触る必要はなく、停止させても転送は動き続ける。**
