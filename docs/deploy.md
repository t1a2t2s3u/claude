...
### 旧LPを閉じる（2026-09-20 本人判断で「削除」に変更）

`akita-gaiheki-mitsumori.com`（屋号「秋田外壁リフォーム本舗」で作った
ランディングページ）は **削除する**。当初は 301 転送の予定だったが、
本人の判断で削除に変更した。

**閉じる理由は評価の分散ではなく、情報の食い違い。** 会社名・住所・電話番号は
新サイトと同じなのに、営業時間（土曜）と対応エリア（「秋田県内」）が
違っていた。同じ事業所を指す2つのページが違うことを書いている状態は、
MEO の減点材料になる。集客の実績も無く、残す理由が無い。
**ページを消せばこの食い違いは解消する。**

- 移したいコンテンツは無い（トップの人物写真はフリー素材だった）
- 広告の着地ページとしては使っていないので、停止しても止まる導線が無い
- 新サイトからも GBP からも、このドメインへのリンクは無い（確認済み）

手順は「**費用が止まる順**」に行う。ドメインを放置してもホスティング代は
出続けるので、**ホスティングの解約が最優先**。

1. ホスティング（DNS は AWS 東京の 13.112.187.226 を指していた）を解約する
2. Search Console に当該プロパティがあれば削除する（任意）
3. ドメインは**自動更新をオフ**にして期限満了で手放す

**ドメインを手放すと第三者が取得できる。** 競合の美工舎塗装工業が
`akita-tosou-mitsumori.com` を使っており、命名の発想が近い。気が変わったら
期限までは引き返せるよう、解約ではなく自動更新オフにしておくこと。
保持する場合は Cloudflare の **Rules → Redirect Rules** で
`https://tatsumi-tosou.com` へ 301 転送できる（LP のサーバーは不要）。

> **対応エリアを広く書かない。** LP の「秋田県内」は、Google に
> 「どこの会社か」を判断させにくくする。範囲を絞るほうが地元の検索で
> 強くなる。正は `site/company.toml` の `areas`。
