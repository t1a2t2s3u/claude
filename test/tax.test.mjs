...
group('配偶者控除・配偶者特別控除', ()=>{
  ...
  /* 控除が0円になったとき、理由が出るか（空文字＝理由なし） */
  const why = (own, sp) => app.spouseReason(own, sp) === '' ? 'なし'
    : app.spouseReason(own, sp).indexOf('あなたの合計所得') === 0 ? '本人の所得' : '配偶者の所得';
  eq('適用されるときは理由なし',   why(5000000, 600000),   'なし');
  eq('配偶者133万ちょうどは理由なし', why(5000000, 1330000), 'なし');
  eq('配偶者140万は配偶者の所得',  why(5000000, 1400000),  '配偶者の所得');
  eq('本人1100万は本人の所得',     why(11000000, 0),       '本人の所得');
  eq('両方だめなら本人を優先',     why(11000000, 3500000), '本人の所得');
});
